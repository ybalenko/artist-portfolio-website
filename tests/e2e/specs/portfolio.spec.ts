...
test.describe("Portfolio", { tag: "@portfolio" }, () => {
  ...
  test("PORTFOLIO-02 metadata and page headings", async ({
    portfolio,
    page,
  }) => {
  ...
  test("PORTFOLIO-06 changing populated section selects its first artwork", async ({
    ...
    await expect(
      portfolio.sectionButton(alternateSection.label),
    ).toHaveAttribute("aria-current", "true");
    await expect(
  ...
  test("PORTFOLIO-08 returning from empty section resets populated selection", async ({
    ...
    await expectImage(portfolio.selectedImage, initial.first);
    await expect(portfolio.thumbnail(0)).toHaveAttribute(
      "aria-current",
      "true",
    );
    await expect(page).toHaveURL(
  ...
  test("PORTFOLIO-16 previous and next synchronize carousel and gallery", async ({
    ...
    await expect(portfolio.thumbnail(1)).toHaveAttribute(
      "aria-current",
      "true",
    );
    await expect(page).toHaveURL(
  ...
  test("PORTFOLIO-23 generated artwork URL restores state in a new page", async ({
    ...
    const sharedPortfolio = new PortfolioPage(sharedPage);
    await sharedPage.goto(selectedUrl);
    await expect(sharedPortfolio.visibleSection).toHaveAttribute(
      "data-portfolio-section",
      initialSection.id,
    );
    await expect(sharedPortfolio.selectedImage).toHaveAttribute(
      "src",
      initial.middle.src,
    );
    await expect(sharedPortfolio.thumbnail(index)).toHaveAttribute(
      "data-index",
      String(index),
    );
  ...
  test("PORTFOLIO-30 shared layout works after populated and empty sections", async ({
    portfolio,
    page,
  }) => {
    await portfolio.goto();
    for (const section of [emptySection, alternateSection]) {
      await portfolio.selectSection(section.label);
      const selectedUrl = page.url();
      await expect(portfolio.header.currentLinks).toHaveText("Portfolio");
      const popup = await portfolio.footer.openFacebook();
      ...
      await portfolio.header.contactsLink.click();
  ...
  test("PORTFOLIO-31 modal isolates shared layout and traps keyboard focus", async ({
    portfolio,
    page,
  }) => {
    await portfolio.goto();
    ...
    for (const control of [
      portfolio.carousel.previousButton,
      portfolio.carousel.nextButton,
      portfolio.carousel.closeButton,
    ]) {
      await page.keyboard.press("Tab");
    ...
    for (const background of [
      portfolio.header.root,
      portfolio.footer.root,
      portfolio.layout.skipLink,
    ]) {
      await expect(background).toHaveJSProperty("inert", true);
    }
  });

  test("PORTFOLIO-32 closing modal restores shared layout without losing artwork", async ({
    portfolio,
    page,
  }) => {
    await portfolio.goto(`#${alternateSection.id}/${alternate.middle.id}`);
    for (const closeWithEscape of [false, true]) {
      ...
      else await portfolio.carousel.close();
      await expect(portfolio.featureButton).toBeFocused();
      for (const background of [
        portfolio.header.root,
        portfolio.footer.root,
      ...
      await expectImage(portfolio.selectedImage, alternate.middle);
      await expectMetadata(portfolio, alternate.middle);
      await portfolio.header.contactsLink.click();
      await expect(page).toHaveURL(/\/contacts\/$/);
      await page.goto(selectedUrl);
    }
